...
from exceptions import TokenError, StatusCodeError, ServerError
...
HW_BOT_ON = 'Бот запущен.'
TOKEN_NOT_FOUND = 'Отсутствует переменная окружения {}.'
TOKEN_ERROR_MESSAGE = 'Отсутствуют переменные окружения.'
...
BASIC_ERROR_MESSAGE = 'Сбой в работе программы: {}.'
...
def get_api_answer(current_timestamp):
    ...
    if response.status_code != HTTPStatus.OK:
        raise StatusCodeError(STATUS_CODE_ERROR_MESSAGE.format(
    ...
    for key in ('error', 'code'):
        if key in response_json:
            raise ServerError(SERVER_ERROR_MESSAGE.format(
                key=key,
...
def parse_status(homework):
    ...
    return HOMEWORK_STATUS_CHANGED.format(homework['homework_name'], verdict)
...
def main():
    ...
    logging.debug(HW_BOT_ON)
    if not check_tokens():
        raise TokenError(TOKEN_ERROR_MESSAGE)
    ...
    while True:
        try:
            ...
            logging.exception(BASIC_ERROR_MESSAGE.format(error))
            try:
                bot.send_message(TELEGRAM_CHAT_ID, message)
            except Exception as error:
